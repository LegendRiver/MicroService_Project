...
class FileHelper
{
    ...
    public static function getSubDirList($dir, $filterKey=array())
    {
        $fileArray = array();
        $allFileNames = scandir($dir);
        ...
        foreach($allFileNames as $fileName)
        {
            if ($fileName != "." && $fileName != "..")
            {
                ...
                if(is_dir($filePath))
                {
                    if(!self::filterDir($fileName, $filterKey))
                    {
                        continue;
                    }
                    $fileArray[] = $filePath;
    ...
    public static function getRecursiveFileList($filePath, $extends = array('.'), $filterDirKey=array())
    {
        $fileList = array();
        if(is_dir($filePath))
        {
            if(!self::filterDir(dirname($filePath), $filterDirKey))
            {
                return $fileList;
            }
    ...
    private static function filterDir($dirName, $filterKeys)
    {
        if(empty($filterKeys))
        {
            return true;
        ...
        foreach ($filterKeys as $filterKey)
        {
            ...
            if(false !== strripos($dirName, $filterKey))
            {
                return false;
            }
        }
    ...
    private static function filterExtend($filePath, $extendArray)
    {
        ...
        {
            if(false !== strripos($filePath, $extend))
            {
                return true;
            }
        }
    ...
    public static function unzipFile($zipFile, $destinationPath)
    {
        $zip = new \ZipArchive;
        $res = $zip->open($zipFile);
        if(true === $res)
        {
            $unzipResult = $zip->extractTo($destinationPath);
            $zip->close();
            return $unzipResult;
        }
        else
        {
            ServerLogger::instance()->writeLog(LogConstant::LOGGER_LEVEL_ERROR, 'The unzip error code is ' . $res);
            return false;
        }
    }
}